...
def train_sgd_log(model, train_dataloader, dev_dataloader):
...
def plot_loss_and_accuracy(train_log, dev_log, file_name):
    fig, ax = plt.subplots(1, 2, figsize=(15, 5))
    ax[0].plot(np.array(train_log).T[0], label='train')
    ax[0].plot(np.array(dev_log).T[0], label='dev')
    ax[0].set_xlabel('epoch')
    ax[0].set_ylabel('loss')
    ax[0].legend()
    ax[1].plot(np.array(train_log).T[1], label='train')
    ax[1].plot(np.array(dev_log).T[1], label='dev')
...
if __name__ == '__main__':
    ...
    train_log, dev_log = train_sgd_log(model, train_dataloader, dev_dataloader)
    plot_loss_and_accuracy(train_log, dev_log, 'SLP_log.png')
