...
class RNN(nn.Module):
    ...
    def padding(self, inputs):
        ...
        '''
        sorted_inputs = sorted(inputs, key=lambda x:x.shape[0], reverse=True)
        tensornn.utils.rnn.pad_sequence(sorted_inputs, batch_first=True)
        '''
    ...
    '''
    def calc_loss_acc(self, model, criterion):
        #train
        outputs = model(self.train_ids)
        #print(outputs)
        train_pred = self.select_class(outputs)
        train_true = self.train_labels.detach().numpy()
        train_loss = criterion(outputs, self.train_labels).detach().numpy().astype(np.float16)
        train_acc = accuracy_score(train_true, train_pred).astype(np.float16)
        #valid
        outputs = model(self.valid_ids)
        valid_pred = self.select_class(outputs)
        valid_true = self.valid_labels.detach().numpy()
        valid_loss = criterion(outputs, self.valid_labels).detach().numpy().astype(np.float16)
        valid_acc = accuracy_score(valid_true, valid_pred).astype(np.float16)
