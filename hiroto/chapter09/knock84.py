...
class RNN(nn.Module):
    def __init__(self, vocab_size, padding_idx, emb_weights=None, embedding_dim=300, hidden_dim=50, category_size=4):
    ...
    def draw(self):
        ...
        plt.savefig("./picture/84lr0.01.png")
...
def main():
    ...
    weights = need_init_weights(True, dic, vocab_size, padding_idx)
    set_seed()
    model = RNN(vocab_size, padding_idx=padding_idx, emb_weights=weights)
    model.to(device)
    model.load_datasets()
