...
class RNN(nn.Module):
    ...
    def Train(self, epoch_size=100, batch_size=80, learning_rate=0.001):
        ...
        model = self
        dataset = MyDataset(self.train_ids, self.train_labels)
        #print(dataset[0:2])
...
def main():
    ...
    model.load_datasets()
    model.Train(epoch_size=1, batch_size=1, learning_rate=0.001)
    model.draw()
...
'''
tensor([[[0.1554, 0.3641, 0.2549, 0.2256]]], grad_fn=<SoftmaxBackward>)
'''